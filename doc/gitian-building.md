...
*Setup instructions for a gitian build of FLS using a Debian VM or physical system.*
...
Clone the git repositories for fls and gitian and then checkout the fls version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/fls-crypto/flits-core.git
cd fls
...
Follow the instructions in [doc/release-process.md](release-process.md) in the fls repository
...
To build FLS (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the fls repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/fls/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/fls-crypto/fls
...
./bin/gbuild --commit fls=${COMMIT} --url fls=${URL} ../fls/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit fls=${COMMIT} --url fls=${URL} ../fls/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit fls=${COMMIT} --url fls=${URL} ../fls/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fls-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/fls-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/fls-build.assert
```
...
[flits-core.gitian.sigs](https://github.com/fls-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
