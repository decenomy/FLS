SSSolutions Core version *3.1.0* is now available from:  <https://github.com/sssolutions-project/sssolutions/releases>
...
SSSolutions Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zsss Staking is here! zsss staking will be activated on the morning of the 8th of May 2018. With the release of zsss staking, there are effectively 2 versions of zsss, zsss minted on the 3.0.6 sssolutions wallet or lower, and zsss minted on sssolutions wallet version or higher. New features in this release will require the use of zsss v2, zsss minted on this wallet release 3.1.0 or later. If you currently hold zsss v1 and wish to take advantage of zsss staking and deterministic zsss, you will need to spend the zsss v1 to yourself and remint zsss v2.
...
To encourage the use of zsss and increase the sssolutions zerocoin anonymity set, the sssolutions payment schedule has been changed to the following:
...
The visual layout of the sssolutions Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zsss minting feature. This is enabled by default and the enablezeromint=0 setting in the sssolutions.conf file will overwrite the GUI option.
...
### Summary
 ...
 The sssolutions wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the sssolutions.conf file, found in the sssolutions data directory.
...
This allows running sssolutionsd without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the sssolutions support group, which often times sees users say "I didn't spend that zsss". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the sssolutions masternodes, the monthly budget available to be utilised has been increased to 42,000 SSS / month. This SSS only has the opportunity to be raised once per month (paid to winning proposals) with any unused SSS not created by the blockchain.

### Budget Finalization Fee

The sssolutions finalization fee for successful proposals has now been reduced, this fee is now 5 SSS down from 50 SSS. The total fee outlay for a successful proposal is now a total of 55 SSS.
...
Several critical security flaws in the zerocoin protocol and sssolutions's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zsss Control dialog in the QT wallet, a user is able to see zsss marked as version 1 or 2.
...
sssolutions's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which sssolutions used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to sssolutions using the bug bounty program. The modulus is now correctly imported and sssolutions's accumulators have been changed to use the new proper modulus.
