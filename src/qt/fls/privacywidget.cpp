...
#include "qt/fls/privacywidget.h"
#include "qt/fls/forms/ui_privacywidget.h"
...
PrivacyWidget::PrivacyWidget(FLSGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zFLS");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zFLS");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zFLS");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zFLS");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zFLS");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zFLS");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zFLS");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zFLS");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zFLS Movements"));
    ...
    // Only Convert to FLS enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzFLS->setTitleClassAndText("btn-title-grey", "Total 0 zFLS");
    ui->btnTotalzFLS->setSubTitleClassAndText("text-subtitle", "Show denominations of zFLS owned.");
    ui->btnTotalzFLS->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select FLS outputs to mint into zFLS.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zFLS");
...
void PrivacyWidget::onMintSelected(bool isMint)
{
    QString btnText;
    if (isMint) {
        btnText = tr("Mint zFLS");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of FLS to mint into zFLS"));
    } else {
        btnText = tr("Convert back to FLS");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zFLS to convert back into FLS"));
    }
...
void PrivacyWidget::onSendClicked()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zFLS is currently undergoing maintenance"));
        return;
    }
    ...
    if (!ctx.isValid()) {
        inform(tr("You need to unlock the wallet to be able to %1 zFLS").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value)
{
    ...
    if (!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)) {
        ...
        // Mint succeed
        inform(tr("zFLS minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value)
{
    ...
    if (!walletModel->convertBackzFLS(
        ...
        // Spend succeed
        inform(tr("zFLS converted back to FLS"));
        // clear
        ui->lineEditAmount->clear();
    }
}
