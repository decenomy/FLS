...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const string& name_, keys) {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid FLS address: ")+name_);
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zflsTracker->ListMints(true, fMatureOnly, true);
...
UniValue listzerocoinamounts(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zflsTracker->ListMints(true, true, true);
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    return DoZflsSpend(nAmount, fMintChange, fMinimizeChange, vMintsSelected, address_str);
}
...
extern UniValue DoZflsSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str)
{
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzFLSTracker* zflsTracker = pwalletMain->zflsTracker.get();
    set<CMintMeta> setMints = zflsTracker->ListMints(false, false, false);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CzFLSTracker* zflsTracker = pwalletMain->zflsTracker.get();
    set<CMintMeta> setMints = zflsTracker->ListMints(!fIncludeSpent, false, false);
