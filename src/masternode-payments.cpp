...
bool IsBlockValueValid(const CBlock& block, CAmount nExpectedValue, CAmount nMinted)
{
    ...
    if (!masternodeSync.IsSynced()) { //there is no budget data to use to check anything
        ...
        if (nHeight % GetBudgetCycleBlocks() < 100) {
            return true;
        } else {
...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZFLSStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (sporkManager.IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZFLSStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZFLSStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
...
void CMasternodePayments::ProcessMessageMasternodePayments(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnget") { //Masternode Payments Request Sync
        ...
        if (winner.nMessVersion != MessageVersion::MESS_VER_HASH && Params().GetConsensus().IsMeFLSigV2(winner.nBlockHeight - 6000)) {
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight)
{
    ...
    const bool fNewSigs = Params().GetConsensus().IsMeFLSigV2(nBlockHeight - 20);
