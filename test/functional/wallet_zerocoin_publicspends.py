...
class ZerocoinSpendTest(flsTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zfls_bal, FLS_bal):
            zfls_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zfls_bal)
            FLS_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], FLS_bal)
            return zfls_bal, FLS_bal
        ...
        zfls_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zfls_balance, 6666)
        ...
        zfls_balance, balance = check_balances(denom_2, zfls_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zfls_balance, balance = check_balances(denom_3, zfls_balance, balance)
        ...
        _, _ = check_balances(denom_1, zfls_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
